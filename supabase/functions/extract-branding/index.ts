...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey) {
      return new Response(
        JSON.stringify({ success: false, error: 'Branding extraction is not configured. Please contact support.' }),
    ...
    // Extract branding from nested response
    const branding = data.data?.branding || data.branding;

    if (!branding) {
      return new Response(
        JSON.stringify({ success: false, error: 'No branding data found for this website' }),
