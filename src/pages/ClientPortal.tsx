...
const ClientPortal = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Validate token
      const { data: tokenData, error: tokenErr } = await supabase.rpc('validate_share_token', { _token: token });
      ...
      const { client_id, org_id } = tokenData[0];

      // Fetch client and org data via security definer functions
      const [clientRes, orgRes] = await Promise.all([
        supabase.rpc('get_portal_client', { _client_id: client_id }),
        supabase.rpc('get_portal_org', { _org_id: org_id }),
      ]);
      ...
      const portalClient = clientRes.data as unknown as PortalClient;
      const portalOrg = orgRes.data as unknown as PortalOrg;

      setClient(portalClient);
      setOrg(portalOrg);
      applyBranding(portalOrg);
      setIsLoading(false);
    };
